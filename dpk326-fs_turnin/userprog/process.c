...
process_execute (const char *file_name) 
{
  ...
  if (child != NULL) {
    ...
    if (thread_current()->path == NULL) {
      /* Initialize process' current directory to root */
      thread_current()->path = (char *)palloc_get_page(0);
      strlcpy(thread_current()->path, "/", 2);
    }
    if (child->path == NULL) {
      /* Initialize child process' current directory */
      child->path = (char *)palloc_get_page(0);
    }
    /* Set child's path to parent's path */
    strlcpy(child->path, thread_current()->path, strlen(thread_current()->path)+1);
  }
